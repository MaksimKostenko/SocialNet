...
if (isset($_GET['add']))
{
    $add = sanitizeString($_GET['add']); // $add содержит имя добавляемого друга
...
for ($j = 0 ; $j < $num ; ++$j)
{
    ...
    $t1 = $result1->num_rows; //1,when smb follow user -->
    
    $result2 = queryMysql("SELECT * FROM friends WHERE
        user='$user' AND friend='" . $row['user'] . "'");
    
    $t2 = $result2->num_rows; // 1,when user follow smb <--
    
    if (($t1 + $t2) > 1) echo " &harr; is a mutual friend";

    elseif ($t2) echo " &larr; you are following";

    elseif ($t1) { echo " &rarr; is following you"; $follow = "accept" ;}
 
    
    if (!$t2) echo " [<a href='members.php?add=" . $row['user'] . "'>$follow</a>]";
